...
const UserProfilePage = ({ navigation }) => {
  let data = {
    ...
    followers: 120,
    following: 80,
    description: ' A passionate developer and tech enthusiast. Always eager to learn new things and share knowledge with others.',
    ...
    posts: [
      { id: 1, postImage: 'https://picsum.photos/400/400' },
      { id: 2, postImage: 'https://picsum.photos/401/400' },
      { id: 3, postImage: 'https://picsum.photos/402/400' },
      { id: 4, postImage: 'https://picsum.photos/403/400' },
    ]
  }
  return (
      ...
      <ScrollView>
        <View style={{ paddingHorizontal: 20 }}>
          <View style={{ alignItems: 'center', marginBottom: 20 }}>
            ...
            />
            <Text style={formHead}>{data.name}</Text>
            <Text style={{ color: 'white' }}>{data.description}</Text>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
            <View style={styles.dataBox}>
              <Text style={{ color: 'white' }}>Followers</Text>
              <Text style={{ color: 'white' }}>{data.followers}</Text>
            </View>
            <View style={styles.dataBox}>
              <Text style={{ color: 'white' }}>Following</Text>
              <Text style={{ color: 'white' }}>{data.following}</Text>
            </View>
            <View style={styles.dataBox}>
              <Text style={{ color: 'white' }}>Posts</Text>
              <Text style={{ color: 'white' }}>{data.posts.length}</Text>
            </View>
          </View>
          <View style={{ marginTop: 20 }}>
            <Text style={formHead}>Your Posts</Text>
            <View style={styles.postContainer}>
              {data.posts.map((post) => (
                <View key={post.id} style={styles.imageWrapper}>
                  <Image
